...
function hideButtons(tab) {
	for (let i = 0; i < tab.length; i++) $('.choose .button_'+tab[i]).parent().hide();
}

function showButtons(tab) {
	for (let i = 0; i < tab.length; i++) $('.choose .button_'+tab[i]).parent().show();
}
...
function mangaBall() {
	...
	let ennemy_name = $('.ennemy .status .name').text(),
		ennemy_id = ennemy.id,
		is_unlocked = (ennemy.unlocked == 1) ? true : false;
	...
	if(!is_unlocked) {
		...
		if(ennemy_id == 26 || ennemy_id == 25 || ennemy_id == 12 || ennemy_id == 29 || ennemy_id == 47 || ennemy_id == 54) {
			...
			let duration = $('.anim_mangaball').css('animation-duration');
