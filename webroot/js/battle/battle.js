...
$(document).ready(function () {
			...
			for (var p = 0; p < _this.response.length; p++) {
				...
				if (_this.response[p].unlocked == 1) {
					...
					if (_this.response[p].condition != 'none' && _this.response[p].condition != null && _this.response[p].condition != 'catch') {
						...
						li_append += '<div class="sub">Vous devez terminer ' + _this.response[p].condition + ' arcades pour débloquer ce personnage !</div>';
					} else {

						li_append += '<div class="sub">Vous devez attraper ce personnage pour pouvoir le jouer !</div>';
					}
	...
	var getUserPersos = function getUserPersos() {
